...
export function createStatusStore() {
  const state = {
    ...
    // Non-secret printer identity (name/brand/model/connectionType/ip/port)
    // as loaded from printer_settings, or null (mock mode, or not loaded
    // yet). Never contains credentials — only what section 17 allows
    // logging.
    printerConfig: null,
  ...
  return {
    ...
    setPrinterConfig(value) {
      state.printerConfig = value;
    },
    setCurrentJob(job) {
